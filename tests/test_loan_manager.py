...
Các test case:
    process_borrow:
        - test_borrow_success                  : U004 (chưa mượn gì) mượn B005 → thành công
        - test_borrow_user_not_found            : "U999" không tồn tại → từ chối
        - test_borrow_book_not_found             : "B999" không tồn tại → từ chối
        - test_borrow_overdue_debt              : U002 (đang nợ quá hạn L002) → từ chối
        - test_borrow_exceed_limit              : U003 (đã mượn đủ 3 cuốn) → từ chối
        - test_borrow_already_borrowing_same_book: U001 mượn lại B001 (đang mượn L001) → từ chối
    ...
    process_return:
        - test_return_no_overdue                : U001 trả B001 (L001) đúng hạn → phí 0
        - test_return_with_overdue_fee           : Trả trễ 3 ngày (SV) → phí 6.000 VNĐ
        - test_return_custom_date               : Trễ 5 ngày → phí 10.000 VNĐ
        - test_return_loan_not_found            : U004 trả B001 (chưa từng mượn) → từ chối
        - test_return_priority_cascade_success  : U001 trả B001 → U004 (hàng chờ B001) được mượn ngay
        - test_return_priority_cascade_ineligible: U003 trả B003 → U002 (hàng chờ B003, đang nợ) vẫn ở lại
        - test_return_priority_cascade_no_match : U003 trả B002 → không ai chờ B002, sách về kho
    ...
    add_to_waiting_queue:
        - test_add_to_waiting_queue_success     : Đăng ký mới (U003/B005) → thành công
        - test_add_to_waiting_queue_duplicate   : U004/B001 đã có sẵn (WR001) → từ chối
...
class LoanManagerTestCase(unittest.TestCase):
    ...
    def test_borrow_success(self):
        ...
        self.assertEqual(len(self.dll.get_transactions_by_user("U004")), 1)
    ...
    def test_return_with_overdue_fee(self):
        ...
        success, msg, fee = loan_manager.process_return(
            self.hash_map, self.dll, self.user_array, self.waiting_queue,
            "U001", "B001", return_date=return_date,
        )
        self.assertTrue(success)
    ...
    def test_return_loan_not_found(self):
        success, msg, fee = loan_manager.process_return(
            self.hash_map, self.dll, self.user_array, self.waiting_queue,
            "U004", "B001", return_date=date(2024, 1, 1),
        )
        self.assertFalse(success)
    ...
    def test_return_priority_cascade_success(self):
        ...
        u004_loans = self.dll.get_transactions_by_user("U004")
    ...
    def test_return_priority_cascade_no_match(self):
        ...
        success, msg, fee = loan_manager.process_return(
            self.hash_map, self.dll, self.user_array, self.waiting_queue,
            "U003", "B002", return_date=return_date,
        )
        self.assertTrue(success)
    ...
    def test_calculate_overdue_fee_not_due_yet(self):
        loan = _find_loan(self.dll, "U001", "L001")
        user = self.user_array.get_by_id("U001")
    ...
    def test_calculate_overdue_fee_returned(self):
        loan = _find_loan(self.dll, "U001", "L006")
        user = self.user_array.get_by_id("U001")
    ...
    def test_add_to_waiting_queue_success(self):
        success, msg = loan_manager.add_to_waiting_queue(
            self.waiting_queue, "U003", "B005", request_date=date(2024, 1, 1)
        )
        self.assertTrue(success)
    ...
    def test_add_to_waiting_queue_duplicate(self):
        success, msg = loan_manager.add_to_waiting_queue(
            self.waiting_queue, "U004", "B001", request_date=date(2024, 1, 1)
        )
        self.assertFalse(success)
        self.assertEqual(self.waiting_queue.size, 3)
    ...
    def test_add_to_waiting_queue_custom_date(self):
        ...
        loan_manager.add_to_waiting_queue(self.waiting_queue, "U002", "B006", request_date=custom_date)
